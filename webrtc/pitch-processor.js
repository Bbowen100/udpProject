...
// Pitch Processor Implementation
class PitchProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      if (event.data.type === 'pitch') {
        ...
        this.bypass = event.data.value;
        // clear internal buffers on bypass switch? Maybe good idea to avoid glitches
        if (this.bypass) {
  ...
  process(inputs, outputs, parameters) {
    ...
    const inputChannel1 = input.length > 1 ? input[1] : input[0];
    const outputChannel0 = output[0];
    const outputChannel1 = output.length > 1 ? output[1] : output[0];
    const bufferSize = inputChannel0.length;

    if (this.samples.length < bufferSize * 2) {
    ...
    for (let i = 0; i < bufferSize; i++) {
      this.samples[i * 2] = inputChannel0[i];
      this.samples[i * 2 + 1] = inputChannel1[i];
    }

    this.soundTouch.inputBuffer.putSamples(this.samples, 0, bufferSize);
    ...
    if (framesToExtract > 0) {
      this.soundTouch.outputBuffer.receiveSamples(this.samples, framesToExtract);
      for (let i = 0; i < framesToExtract; i++) {
        outputChannel0[i] = this.samples[i * 2];
        outputChannel1[i] = this.samples[i * 2 + 1];
      }
    }
